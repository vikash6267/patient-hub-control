...
export interface CallHistoryRecord {
  ...
  type: "Voice" | "Fax" | "SMS"
  direction: "Inbound" | "Outbound"
  action: "Phone Call" | "Ring Out" | "RingMe" | "Conference Call" | "Greeting" | "Incoming Fax" | "Outgoing Fax"
  result:
    | "Completed"
    | "Missed"
    | "Call accepted"
...
class RingCentralStore {
  ...
  private updateStates() {
    ...
    if (this.authType === "jwt") {
      ...
      hasValidTokens = !!(this.rcToken && this.refreshToken && this.tokenExpiryTime > Date.now())
  ...
  public async connect() {
    ...
    if (this.rcToken && this.tokenExpiryTime > Date.now() + 5 * 60 * 1000) {
      ...
      try {
        ...
        message.success(`✅ Reconnected as ${this.extInfo?.contact?.firstName} ${this.extInfo?.contact?.lastName}`)
    ...
    try {
      ...
      if (error.message?.includes("Too Many Contacts")) {
        message.error("❌ Too many active sessions. Please wait 5 minutes and try again, or use a different device.")

        // Try to cleanup and retry once
        await this.cleanupConnections()
        await this.clearStoredCredentials()

        message.info("🔄 Cleaned up sessions. Please try connecting again in a few minutes.")
      } else {
  ...
  private async fetchUserInfo() {
    ...
    const numberList = await this.rc.restapi().account().extension().phoneNumber().get()
  ...
  private async initializeWebPhone() {
    if (!this.rc || !this.extInfo) {
      throw new Error("Cannot initialize WebPhone without RC client and extension info")
    }
...
let response;
    ...
    try {
      ...
      response = await this.rc.get("/restapi/v1.0/account/~/extension/~/call-log", {
        params: {
      ...
      try {
        const callLogEndpoint = this.rc.restapi().account().extension().callLog();
    ...
    const newRecords: CallHistoryRecord[] = records.map((record: any) => ({
      ...
      recording: record.recording
        ? {
            ...
            type: (record.recording.type || "Automatic") as "Automatic" | "OnDemand",
            contentUri: record.recording.contentUri!,
            duration: record.recording.duration,
          }
        : undefined,
      transport: (record.transport || "PSTN") as "PSTN" | "VoIP",
      lastModifiedTime: record.lastModifiedTime || record.startTime || new Date().toISOString(),
    ...
    // ✅ Paging logic
    const totalElements = paging.totalElements || (this.callHistoryPage * this.callHistoryPerPage);
    ...
    const dummyRecords: CallHistoryRecord[] = [
      {
        ...
        action: "Phone Call",
        result: "Completed",
        to: { phoneNumber: "+1234567890", name: "Test User", location: "NYC" },
  ...
  public getCallStats() {
    ...
    const incoming = this.callHistory.filter((c) => c.direction === "Inbound").length
    const outgoing = this.callHistory.filter((c) => c.direction === "Outbound").length
...
async downloadRecording(recordingId: string): Promise<string | null> {
  try {
    ...
    const response = await this.rc.get(`/restapi/v1.0/account/~/recording/${recordingId}`);
    ...
    if (!blobResponse.ok) {
      console.error("Failed to fetch recording content:", blobResponse.statusText);
      return null;
    }
  ...
  public async makeCall(number: string, patientName?: string): Promise<CallSession | null> {
    if (!this.isLoggedIn) {
    ...
    try {
      ...
      const callSession = this.wrapCallSession(rawCallSession, "outbound", patientName)
  ...
  private wrapCallSession(rawSession: any, direction: "inbound" | "outbound", patientName?: string): CallSession {
  ...
  private removeActiveCall(callSession: CallSession) {
    ...
    this.activeCalls = this.activeCalls.filter((call) => call.callId !== callSession.callId)
  ...
  private setupTokenRefresh() {
    ...
    const refreshToken = async () => {
      if (this.rcToken !== "" && this.isConnected && this.rc && this.refreshToken) {
        try {
          await this.rc.refresh()
          this.rcToken = this.rc.token!.access_token!
          this.refreshToken = this.rc.token!.refresh_token!
